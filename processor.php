...
if (isset($_POST["detectDateMode"])) {
    if ($_POST["detectDateMode"] === "1") {
        $detectDateMode = 1;
    } else if ($_POST["detectDateMode"] === "2") {
        $detectDateMode = 2;
    }
}
if (isset($_POST["addSerializedNames"])) {
    $addSeriazedNames = $_POST["addSerializedNames"];
}
...
if ($toFile) {
    ...
    $zipPath = "./data/" . $uuid . '.zip';
    $zip = createZipFromDir($path, $zipPath);
    echo $zipPath;
} else {
    $map->show();
}
